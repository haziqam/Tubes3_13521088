"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.deleteQnA = exports.updateAnswer = exports.addQnA = exports.getQnA = void 0;
function getQnA() {
    return __awaiter(this, void 0, void 0, function () {
        var res, data;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, fetch("http://localhost:3000/api/post/getQnA")];
                case 1:
                    res = _a.sent();
                    if (!res.ok) {
                        throw new Error('Failed to fetch data from API');
                    }
                    return [4 /*yield*/, res.json()];
                case 2:
                    data = _a.sent();
                    return [2 /*return*/, data];
            }
        });
    });
}
exports.getQnA = getQnA;
/**
 * @param question Pertanyaan yang akan ditambahkan
 * @param answer Jawaban yang akan ditambahkan
 * @returns Data QuestionAndAnswer {id, question, answer} yang berhasil ditambahkan
 */
function addQnA(question, answer) {
    return __awaiter(this, void 0, void 0, function () {
        var res, ApiResponseMsg, addedQnA;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, fetch("http://localhost:3000/api/post/addQnA", {
                        method: 'POST',
                        body: JSON.stringify({ param: {
                                question: question,
                                answer: answer
                            }
                        })
                    })];
                case 1:
                    res = _a.sent();
                    if (!!res.ok) return [3 /*break*/, 3];
                    return [4 /*yield*/, res.text()];
                case 2:
                    ApiResponseMsg = _a.sent();
                    throw new Error('Failed to add data using API: ' + ApiResponseMsg);
                case 3: return [4 /*yield*/, res.json()];
                case 4:
                    addedQnA = _a.sent();
                    return [2 /*return*/, addedQnA];
            }
        });
    });
}
exports.addQnA = addQnA;
function updateAnswer(id, newAnswer) {
    return __awaiter(this, void 0, void 0, function () {
        var res, ApiResponseMsg, updatedQnA;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, fetch("http://localhost:3000/api/post/updateAnswer", {
                        method: 'PUT',
                        body: JSON.stringify({ param: {
                                id: id,
                                newAnswer: newAnswer
                            }
                        })
                    })];
                case 1:
                    res = _a.sent();
                    if (!!res.ok) return [3 /*break*/, 3];
                    return [4 /*yield*/, res.text()];
                case 2:
                    ApiResponseMsg = _a.sent();
                    throw new Error('Failed to update data using API: ' + ApiResponseMsg);
                case 3: return [4 /*yield*/, res.json()];
                case 4:
                    updatedQnA = _a.sent();
                    return [2 /*return*/, updatedQnA];
            }
        });
    });
}
exports.updateAnswer = updateAnswer;
function deleteQnA(id) {
    return __awaiter(this, void 0, void 0, function () {
        var res, ApiResponseMsg, deletedQnA;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, fetch("http://localhost:3000/api/post/deleteQnA", {
                        method: 'PUT',
                        body: JSON.stringify({ param: {
                                id: id
                            }
                        })
                    })];
                case 1:
                    res = _a.sent();
                    if (!!res.ok) return [3 /*break*/, 3];
                    return [4 /*yield*/, res.text()];
                case 2:
                    ApiResponseMsg = _a.sent();
                    throw new Error('Failed to delete data using API: ' + ApiResponseMsg);
                case 3: return [4 /*yield*/, res.json()];
                case 4:
                    deletedQnA = _a.sent();
                    return [2 /*return*/, deletedQnA];
            }
        });
    });
}
exports.deleteQnA = deleteQnA;
// let test: QuestionAndAnswer[];
// getQnA().then((allQnA) => {
//   test = allQnA;
//   console.log(test);
// })
// try {
//   updateAnswer(10, "hhhhhhhhh")
//     .then((updatedAnswer) => {
//       console.log(updatedAnswer)
//     })
// }
// catch (error) {
//   console.log(error)
// }
// try {
//   addQnA("asdf", "jkl;")
//     .then((addedData) => {
//       console.log(addedData);
//     });
// }
// catch (error) {
//   console.error(error);
// };
try {
    deleteQnA(1)
        .then(function (deletedData) {
        console.log(deletedData);
    });
}
catch (error) {
    console.error(error);
}
;
